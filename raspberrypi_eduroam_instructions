...
* USB mouse, keyboard and screen for simplest setup

---

## Instructions:
* Flash the SD card with the latest version of Raspbian [Instructions here](https://www.raspberrypi.org/downloads/)
* Boot up the Raspberry Pi for the first time, with keyboard, mouse, monitor and ethernet cable connected
...
* Now open your browser and navigate to https://cat.eduroam.org
* Click the lower box labelled "download your eduroam installer"
* A pop-up will appear with a list of institutions, select Cardiff Metropolitan University (if you are based at another institution, try to find your institution from the list, failing this please consult with your IT department). Accept the download.
...
```
* Note that your login and password will be exposed in this document! We will fix that shortly.
...
* So far so good, let's secure that password for peace of mind. In terminal, type the following command: ```echo -n yourPasswordHere | iconv -t utf16le | openssl md4```
